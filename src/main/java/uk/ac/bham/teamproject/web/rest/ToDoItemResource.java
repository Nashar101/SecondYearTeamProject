...
/**
 * REST controller for managing {@link uk.ac.bham.teamproject.domain.ToDoItem}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ToDoItemResource {
    ...
    public ResponseEntity<ToDoItem> partialUpdateToDoItem(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody ToDoItem toDoItem
    ) throws URISyntaxException {
        ...
        Optional<ToDoItem> result = toDoItemRepository
            ...
            .map(existingToDoItem -> {
                if (toDoItem.getToDoItemHeading() != null) {
                    existingToDoItem.setToDoItemHeading(toDoItem.getToDoItemHeading());
                }
                if (toDoItem.getToDoItemDescription() != null) {
                    existingToDoItem.setToDoItemDescription(toDoItem.getToDoItemDescription());
                }
                if (toDoItem.getToDoItemStatus() != null) {
                    existingToDoItem.setToDoItemStatus(toDoItem.getToDoItemStatus());
                }
